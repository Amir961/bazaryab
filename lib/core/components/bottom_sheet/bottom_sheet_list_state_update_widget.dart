...
class ListStateUpdateWidget extends StatefulWidget {


  final BuildContext context;

  const ListStateUpdateWidget({super.key,required this.context});

  @override
  State<ListStateUpdateWidget> createState() => _ListStateUpdateWidgetState();
}



class _ListStateUpdateWidgetState extends State<ListStateUpdateWidget> {
  ...
  start()async
  {
    ...
    if(BlocProvider.of<UpdateCustomerBloc>(widget.context).state.listState.isEmpty) {
      BlocProvider.of<UpdateCustomerBloc>(widget.context).add(GetStateEvent());
    }
    else {
      selectCity = BlocProvider
          .of<UpdateCustomerBloc>(widget.context)
          .state
          .selectedState;
                ...
                child: Stack(
                  children: [
                    ...
                    Positioned(
                        top:80,
                        right: 20,
                        child: MyText(text: "تعیین استان",fontWeight: FontWeight.bold,fontSize: 18,color: Colors.white,)),
                  ],
                ),
              ),
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    SizedBox(height: 20,),
                    MyText(text: 'استان مورد نظر را انتخاب کنید'),
                    SizedBox(height: 20,),
                    Padding(
  ...
  Widget get _listView => BlocConsumer<UpdateCustomerBloc,UpdateCustomerState>(
      ...
      listener: (context,state){
        ...
        if(state.statusButtonGetState== StatusButton.noInternet)
        {
       ...
       else if(state.statusButtonGetState== StatusButton.success)
        {

          originList=BlocProvider.of<UpdateCustomerBloc>(widget.context).state.listState;
          tempList=List.from(BlocProvider.of<UpdateCustomerBloc>(widget.context).state.listState);

        }
       else if(state.statusButtonGetState== StatusButton.failed)
        {
      ...
      buildWhen: (previous, current) => previous.statusButtonGetState != current.statusButtonGetState,
      builder: (context,state){

    return state.statusButtonGetState==StatusButton.loading? Center(child: Column(
    ...
    ),): state.statusButtonGetState==StatusButton.success && tempList.isEmpty? Center(child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      mainAxisSize: MainAxisSize.max,
      children: [
